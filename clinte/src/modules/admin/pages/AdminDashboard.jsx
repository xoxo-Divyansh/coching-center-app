...
const AdminDashboard = () => {
 ...
 const loadDashboard = async () => {
  try {
    ...
    // Parallel calls for 80% faster loading
    const [statsRes, revenueRes, usersRes, teacherRequestRes] = await Promise.allSettled([
      getAdminStats(),
      getAdminRevenue(), 
      getAdminUsers(),
      getTeacherRequests().catch(() => ({ data: { requests: [] } })) // Graceful fallback
    ]);

    console.log("ALL API CALLS COMPLETED:", { statsRes, revenueRes, usersRes, teacherRequestRes });

    // Safe data extraction with fallbacks
    setStats(statsRes.status === 'fulfilled' ? statsRes.value.data?.stats || {} : {});
    setRevenue(revenueRes.status === 'fulfilled' ? revenueRes.value.data?.revenue || {} : {});
    setUsers(usersRes.status === 'fulfilled' ? usersRes.value.data?.users || [] : []);
    
    const teacherRequests = teacherRequestRes.status === 'fulfilled' 
      ? teacherRequestRes.value.data?.requests || [] 
      : [];
    setPendingTeacherRequests(teacherRequests.filter(req => req.status === "pending").length);

  } catch (err) {
    console.error("CRITICAL DASHBOARD ERROR:", err); // Better logging
